...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if img is None or mask is None:
            raise FileNotFoundError(f"Imaginea {self.image_paths[idx]} sau masca lipsesc.")
...
class UNet(nn.Module):
    def __init__(self):
        super().__init__()
        weights = torch.load("/content/simclr_backbone_resnet50_light.pth", map_location="cpu")
